...
import { View, TouchableOpacity, StatusBar, TouchableWithoutFeedback } from 'react-native';
...
export default function Movie(movie) {
          ...
          {state.showControls && (
            <View style={styles.controlOverlay}>
              {state.allowFullscreen ? (
                <TouchableOpacity onPress={handleFullscreen} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }} style={styles.fullscreenButton}>
                  {state.fullscreen ? <Icon name="minimize" type="Feather" style={styles.icon} /> : <Icon name="maximize" type="Feather" style={styles.icon} />}
                </TouchableOpacity>
              ) : null}

              <PlayerControls onPlay={handlePlayPause} onPause={handlePlayPause} playing={state.play} showPreviousAndNext={false} showSkip skipBackwards={skipBackward} skipForwards={skipForward} />
              <ProgressBar
  ...
  function showControls() {
    state.showControls ? setState({ ...state, showControls: false }) : setState({ ...state, showControls: true });
  }
}
