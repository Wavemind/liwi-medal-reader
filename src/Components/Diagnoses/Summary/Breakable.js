...
const Breakable = ({ drug, drugDose }) => {
  ...
  return (
    <View>
      ...
      {drugDose.by_age ? (
        <Text style={Fonts.textSmall}>{`${roundSup(drugDose.unique_dose)} ${t('formulations.drug.tablets')} ${t('formulations.medication_form.per_administration')} ${t('formulations.drug.every')} ${drugDose.recurrence} ${t('formulations.drug.h')} ${t('formulations.drug.during')} ${drug.duration} ${t('formulations.drug.days')}`}</Text>
      ) : drugDose.doseResult === null ? (
        <Text style={Fonts.textSmall}>{drugDose.no_possibility}</Text>
      ) : (
        <View>
          <Text style={Fonts.textSmall}>
            {`${t('formulations.drug.give')} ${drugDose.doseResult * (drugDose.dose_form / drugDose.breakable)} ${t('formulations.drug.mg')} : ${fractionString} ${t('formulations.drug.tablet')} ${drugDose.dose_form}`}
